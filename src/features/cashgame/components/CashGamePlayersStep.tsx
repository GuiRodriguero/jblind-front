...
interface CashGamePlayersProps {
  readonly players: CashGamePlayer[];
  readonly onPlayersChange: (players: CashGamePlayer[]) => void;
}

export function CashGamePlayersStep({ players, onPlayersChange }: CashGamePlayersProps) {
  const { t } = useTranslation();

  const addPlayer = () => {
    onPlayersChange([...players, { id: crypto.randomUUID(), name: '', buyIn: 0 }]);
  };
  ...
  return (
    <div className="animate-in fade-in slide-in-from-right-4 duration-300">
      <div className="flex items-center gap-2 text-blue-400 mb-6 border-b border-white/10 pb-4">
        <Users size={18} />
        <h2 className="font-bold uppercase tracking-widest text-xs">{t('cashgame.new.players.title')}</h2>
      </div>
      ...
      <div className="flex flex-col gap-3">
        {players.map((player, index) => (
          <div key={player.id} className="flex items-center gap-3">
            ...
            <button
              ...
              aria-label={t('cashgame.new.players.remove')}
            >
              <Trash2 size={16} />
            </button>
          </div>
        ...
        {players.length === 0 && (
          <div className="p-10 text-center text-gray-600 text-sm italic">{t('cashgame.new.players.empty')}</div>
        )}
      </div>
      ...
      >
        <Plus size={14} /> {t('cashgame.new.players.addPlayer')}
      </button>
    </div>
  );
}
